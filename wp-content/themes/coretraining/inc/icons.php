<?php
/**
 * Inline SVG icons (stroke, 24×24)
 */

function coretraining_icon(string $name, array $attrs = []): string {
    $paths = [
    ...
    $class = trim('ct-icon ' . ($attrs['class'] ?? ''));

    return sprintf(
        '<svg class="%s" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">%s</svg>',
        esc_attr($class),
        $paths[$name]
    );
}
